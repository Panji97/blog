...
function tagsFromInput(input?: string): string[] {
  if (!input) return [];
  return input.split(",").map((t) => t.trim()).filter(Boolean).slice(0, 8);
}
...
export async function createPostAction(formData: FormData) {
  ...
  const raw = {
    ...
    slug: String(formData.get("slug") ?? "") || slugifyTitle(String(formData.get("title") ?? "")),
...
export async function updatePostAction(id: string, formData: FormData) {
  ...
  const clash = await db.select().from(posts).where(eq(posts.slug, parsed.data.slug)).limit(1);
  if (clash.length && clash[0].id !== id) return { error: "Slug is already in use." };
  const now = new Date().toISOString();
...
export async function togglePublishAction(id: string) {
  ...
  await db.update(posts).set({
    status: next,
    publishedAt: next === "published" ? cur[0].publishedAt ?? now : cur[0].publishedAt,
    updatedAt: now,
  }).where(eq(posts.id, id));
  revalidatePath("/");
  revalidatePath("/blog");
...
export async function upsertCategoryAction(id: string | null, formData: FormData) {
  ...
  const raw = {
    ...
    slug: String(formData.get("slug") ?? "") || slugifyTitle(String(formData.get("name") ?? "")),
  ...
  const db = getDb();
  if (id) {
    await db.update(categories).set({ name: parsed.data.name, slug, description: parsed.data.description ?? null }).where(eq(categories.id, id));
  } else {
    await db.insert(categories).values({ id: uid("c_"), name: parsed.data.name, slug, description: parsed.data.description ?? null, createdAt: new Date().toISOString() }).onConflictDoNothing();
  }
  revalidatePath("/");
  revalidatePath("/blog");
  return { ok: true };
